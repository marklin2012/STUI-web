...
function FilterDropdown<RecordType>(props: FilterDropdownProps<RecordType>) {
  ...
  const onCheck = (checked: Key[] | { checked: Key[]; halfChecked: Key[] }, info: any) => {
    if (!filterMultiple) {
      onSelectKeys({ selectedKeys: info.checked && info.node.key ? [info.node.key] : [] })
    } else {
      if (typeof checked === 'object') {
        const temp = checked as { checked: Key[]; halfChecked: Key[] }
        onSelectKeys({ selectedKeys: temp.checked as Key[] })
      } else {
        onSelectKeys({ selectedKeys: checked as Key[] })
      }
    }
  }
  ...
  if (typeof column.filterIcon === 'function') {
    ...
    filterIcon = prefixClsIcon(<Icon icon={'filter'} />)
  }
