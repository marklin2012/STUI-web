...
function injectSorter<RecordType>(
  ...
  return (columns || []).map((column, index) => {
    ...
    if (newColumn.sorter) {
      ...
      const upPrefixCls = classNames(`${prefixCls}-column-sorter-up`, {
        active: sorterOrder === ASCEND,
      })
      const upNode: React.ReactNode =
        sortDirections.includes(ASCEND) && prefixClsIcon(<Icon icon={'caret-up'} />, upPrefixCls)
      const downPrefixCls = classNames(`${prefixCls}-column-sorter-down`, {
        active: sorterOrder === DESCEND,
      })
      const downNode: React.ReactNode =
        sortDirections.includes(DESCEND) &&
        prefixClsIcon(<Icon icon={'caret-down'} />, downPrefixCls)
