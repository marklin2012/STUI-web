...
const prefixCls = 'st-scroll-number'
...
const ScrollNumber: React.FC<ScrollNumberProps> = ({
  ...
  const [animateStareted, setAnimateStarted] = React.useState(true)
  ...
  }, [animateStareted, customizeCount, onAnimated]) // 仅在第二个参数更改时更新
  ...
  const renderCurrentNumber = (num: number | string, i: number) => {
    if (typeof num === 'number') {
      ...
      const removeTransition = animateStareted || getNumberArray(lastCount)[i] === undefined
  ...
  /// 去掉1位数时的padding
  if ((count && Number(count) < 10) || count === '0' || count === 0) {
    newProps.style = {
      ...style,
      padding: 0,
    }
