...
import type { UploadProps, RcFile } from '../upload'
import { PREFIX, INIT_ZOOM, INIT_ROTATE } from './constants'
import EasyCrop, { EasyCropHandle } from './easy-crop'
...
const ImgCrop = forwardRef<Cropper, ImgCropProps>((props, ref) => {
  ...
  const modalProps = useMemo(() => {
    ...
    Object.keys(obj).forEach(() => {
      ...
      if (!obj.okText) delete obj.okText
      if (!obj.cancelText) delete obj.cancelText
    })
    return obj
  ...
  const onOk = useCallback(async () => {
    ...
    if (rotate && easyCropRef.current.rotateVal !== INIT_ROTATE && !ctx) {
