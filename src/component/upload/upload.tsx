...
import { UploadProgressEvent, RcFile as OriRcFile } from 'rc-upload/lib/interface'
...
const InternalUpload: React.ForwardRefRenderFunction<unknown, UploadProps> = (props, ref) => {
  ...
  const onProgress = (e: UploadProgressEvent, file: OriRcFile) => {
    const newFile: RcFile = { ...file }
    // removed
    if (!getFileItem(newFile, mergedFileList)) {
      return
    }

    const targetItem = file2Obj(newFile)
  ...
  function getShowUploadList(button?: React.ReactNode, buttonVisible?: boolean): React.ReactNode {
    let tempShowUploadList: ShowUploadListInterface
    if (typeof showUploadList === 'boolean') {
      tempShowUploadList = {}
    } else {
      tempShowUploadList = showUploadList as ShowUploadListInterface
    }
    const {
      showRemoveIcon,
      showPreviewIcon,
    ...
    return (
      <UploadList
        ...
        locale={{ ...defaultLocale, ...propLocale }}
  ...
  const renderUploadList = (button?: React.ReactNode, buttonVisible?: boolean) =>
    showUploadList ? getShowUploadList(button, buttonVisible) : button
